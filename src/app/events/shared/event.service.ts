...
export class EventService {
  ...
  getEvents(): Observable<IEvent[]> {
    ...
    // pipe(catchError...) is to catch error. to do that we need to pass the handleError method with return type that is type of the getEvents method;
    // to pass the operation arguments is for error purposes to pass the name of the method where the error comes from and default result as empty array
  }
...
// due to our smart endpoint, if there is an ID it will update by using post and if there is no ID it will save as a new ID. so we dont need to use here put (update event), post event will do both;
  saveEvent(event) {
...
// commented code is working code for mocked data;
    ...
    // no need of the above code since we can use query string ;
